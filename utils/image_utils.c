...
static const char* filter_image_names[] = {
    "jpg", "jpeg", "JPG", "JPEG", "png", "PNG", "data", NULL};
...
static int read_image_jpeg(const char* path, image_buffer_t* image)
{
    ...
    ret              = tjDecompressHeader3(handle,
                              jpegBuf,
                              size,
                              &origin_width,
                              &origin_height,
    ...
    {
        sw_out_buf =
            (unsigned char*)malloc(sw_out_size * sizeof(unsigned char));
    }
    if (sw_out_buf == NULL)
    {
...
static int
write_image_jpeg(const char* path, int quality, const image_buffer_t* image)
{
    int                  ret;
...
int read_image(const char* path, image_buffer_t* image)
{
    ...
    else if (strcmp(_ext, ".jpg") == 0 || strcmp(_ext, ".jpeg") == 0 ||
...
static int crop_and_scale_image_yuv420sp(unsigned char* src,
                                         int            src_width,
                                         int            src_height,
                                         int            crop_x,
                                         int            crop_y,
...
static int convert_image_cpu(image_buffer_t* src,
                             image_buffer_t* dst,
                             ...
                             char            color)
{
    ...
    {
        reti = crop_and_scale_image_yuv420sp(src->virt_addr,
                                             src->width,
                                             src->height,
                                             src_box_x,
                                             src_box_y,
                             ...
                             char            color)
{
    ...
    if (use_handle)
    {
        ...
        {
            rga_handle_src =
                importbuffer_physicaladdr((uint64_t)src_phy, &in_param);
        }
        else if (src_fd > 0)
        {
    ...
    if (use_handle)
    {
        ...
        {
            rga_handle_dst =
                importbuffer_physicaladdr((uint64_t)dst_phy, &dst_param);
        }
        else if (dst_fd > 0)
        {
...
int convert_image(image_buffer_t* src_img,
                  image_buffer_t* dst_img,
                  image_rect_t*   src_box,
                  image_rect_t*   dst_box,
                  char            color)
