...
int init_yolov8_model(const char* model_path, rknn_app_context_t* app_ctx)
{
    ...
    app_ctx->input_mems[0] =
        rknn_create_mem(ctx, input_native_attrs[0].size_with_stride);
    ...
    for (uint32_t i = 0; i < io_num.n_output; ++i)
    {
        app_ctx->output_mems[i] =
            rknn_create_mem(ctx, output_native_attrs[i].size_with_stride);
        ret = rknn_set_io_mem(
            ctx, app_ctx->output_mems[i], &output_native_attrs[i]);
        if (ret < 0)
        {
    ...
    app_ctx->input_native_attrs =
        (rknn_tensor_attr*)malloc(io_num.n_input * sizeof(rknn_tensor_attr));
    memcpy(app_ctx->input_native_attrs,
           input_native_attrs,
           io_num.n_input * sizeof(rknn_tensor_attr));
    app_ctx->output_native_attrs =
        (rknn_tensor_attr*)malloc(io_num.n_output * sizeof(rknn_tensor_attr));
    memcpy(app_ctx->output_native_attrs,
           output_native_attrs,
           io_num.n_output * sizeof(rknn_tensor_attr));
        ...
        for (int c = 0; c < channel; ++c)
        {
            ...
            for (int cur_h = 0; cur_h < h; ++cur_h)
            {
                for (int cur_w = 0; cur_w < w; ++cur_w)
                {
                    ...
                    dst_b[c * hw_dst + cur_hw] =
                        src_bc[C2 * cur_hw + offset]; // int8-->int8
                }
            }
        }
    }
                           ...
                           object_detect_result_list* od_results)
{
    ...
    for (uint32_t i = 0; i < app_ctx->io_num.n_output; i++)
    {
        ...
        {
            outputs[i].size =
                app_ctx->output_native_attrs[i].n_elems * sizeof(int8_t);
